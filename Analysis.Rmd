...
```{r}
#Read in the data
HB17_background <- Read10X(data.dir = 'data/HB17_background_raw_feature_bc_matrix')
HB17_background <- CreateSeuratObject(counts =HB17_background, project = "HB17_background", min.cells = 3, min.features = 200)
HB17_background_cells_before <- ncol(HB17_background)
HB17_background_genes_before <- nrow(HB17_background)

HB17_PDX <- Read10X(data.dir = 'data/HB17_PDX_raw_feature_bc_matrix')
HB17_PDX <- CreateSeuratObject(counts = HB17_PDX, project = "HB17_PDX", min.cells = 3, min.features = 200)
HB17_PDX_cells_before <- ncol(HB17_PDX)
HB17_PDX_genes_before <- nrow(HB17_PDX)

HB17_tumor <- Read10X(data.dir = 'data/HB17_tumor_raw_feature_bc_matrix')
HB17_tumor <- CreateSeuratObject(counts =HB17_tumor, project = "HB17_tumor", min.cells = 3, min.features = 200)
HB17_tumor_cells_before <- ncol(HB17_tumor)
HB17_tumor_genes_before <- nrow(HB17_tumor)

HB30_PDX <- Read10X(data.dir = 'data/HB30_PDX_raw_feature_bc_matrix')
HB30_PDX <- CreateSeuratObject(counts = HB30_PDX, project = "HB30_PDX", min.cells = 3, min.features = 200)
HB30_PDX_cells_before <- ncol(HB30_PDX)
HB30_PDX_genes_before <- nrow(HB30_PDX)

HB30_tumor <- Read10X(data.dir = 'data/HB30_tumor_raw_feature_bc_matrix')
HB30_tumor <- CreateSeuratObject(counts = HB30_tumor, project = "HB30_tumor", min.cells = 3, min.features = 200)
HB30_tumor_cells_before <- ncol(HB30_tumor)
HB30_tumor_genes_before <- nrow(HB30_tumor)

HB53_background <- Read10X(data.dir = 'data/HB53_background_raw_feature_bc_matrix')
HB53_background <- CreateSeuratObject(counts = HB53_background, project = "HB53_background", min.cells = 3, min.features = 200)
HB53_background_cells_before <- ncol(HB53_background)
HB53_background_genes_before <- nrow(HB53_background)

HB53_tumor <- Read10X(data.dir = 'data/HB53_tumor_raw_feature_bc_matrix')
HB53_tumor <- CreateSeuratObject(counts = HB53_tumor, project = "HB53_tumor", min.cells = 3, min.features = 200)
HB53_tumor_cells_before <- ncol(HB53_tumor)
HB53_tumor_genes_before <- nrow(HB53_tumor)
...
HB17_background <- subset(HB17_background, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
HB17_PDX <- subset(HB17_PDX, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
HB17_tumor <- subset(HB17_tumor, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
HB30_PDX <- subset(HB30_PDX, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
HB30_tumor <- subset(HB30_tumor, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
HB53_background <- subset(HB53_background, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
HB53_tumor <- subset(HB53_tumor, subset = nFeature_RNA >500 & nCount_RNA > 800 & percent.mt < 10 )
...
```{r}
#HB17_background Doublet Process
...
#estimate expected number of doublets based on Chromium10 
nCells <- ncol(HB17_background)
doublet_rate <- 0.01  # assuming 1%
nExp <- round(nCells * doublet_rate)


# Run DoubletFinder with optimal parameters
HB17_background <- doubletFinder(HB17_background, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)


# Identify the column storing DoubletFinder classification
col_name <- colnames(HB17_background@meta.data)[8]

#output doublet plot/table
DimPlot(HB17_background, reduction = "umap.unintegrated", group.by = col_name)
table(HB17_background@meta.data$col_name)


# Create a logical vector for singlets
singlet_cells <- rownames(HB17_background@meta.data)[
  HB17_background@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB17_background <- HB17_background[, singlet_cells]
```

```{r}
#HB17_PDX Doublet Process
...
doublet_rate <- 0.01  # assume 1%
nExp <- round(nCells * doublet_rate)

# Run DoubletFinder with optimal parameters
HB17_PDX <- doubletFinder(HB17_PDX, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)

# Identify the column storing DoubletFinder classification
col_name <- colnames(HB17_PDX@meta.data)[8]

#output doublet plot/table
DimPlot(HB17_PDX, reduction = "umap.unintegrated", group.by = col_name)
table(HB17_PDX@meta.data$col_name)

# Create a logical vector for singlets
singlet_cells <- rownames(HB17_PDX@meta.data)[
  HB17_PDX@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB17_PDX <- HB17_PDX[, singlet_cells]
```


```{r}
#HB17_tumor Doublet Process
...
# Run DoubletFinder with optimal parameters
HB17_tumor <- doubletFinder(HB17_tumor, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)

# Identify the column storing DoubletFinder classification
col_name <- colnames(HB17_tumor@meta.data)[8]

#output doublet plot/table
DimPlot(HB17_tumor, reduction = "umap.unintegrated", group.by = col_name)
table(HB17_tumor@meta.data$col_name)

# Create a logical vector for singlets
singlet_cells <- rownames(HB17_tumor@meta.data)[
  HB17_tumor@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB17_tumor <- HB17_tumor[, singlet_cells]
```


```{r}
#HB30_PDX Doublet Process
...
# Run DoubletFinder with optimal parameters
HB30_PDX <- doubletFinder(HB30_PDX, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)

# Identify the column storing DoubletFinder classification
col_name <- colnames(HB30_PDX@meta.data)[8]

#output doublet plot/table
DimPlot(HB30_PDX, reduction = "umap.unintegrated", group.by = col_name)
table(HB30_PDX@meta.data$col_name)

# Create a logical vector for singlets
singlet_cells <- rownames(HB30_PDX@meta.data)[
  HB30_PDX@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB30_PDX <- HB30_PDX[, singlet_cells]
```


```{r}
#HB30_tumor Doublet Process
...
# Run DoubletFinder with optimal parameters
HB30_tumor <- doubletFinder(HB30_tumor, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)

# Identify the column storing DoubletFinder classification
col_name <- colnames(HB30_tumor@meta.data)[8]

#output doublet plot/table
DimPlot(HB30_tumor, reduction = "umap.unintegrated", group.by = col_name)
table(HB30_tumor@meta.data$col_name)

# Create a logical vector for singlets
singlet_cells <- rownames(HB30_tumor@meta.data)[
  HB30_tumor@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB30_tumor <- HB30_tumor[, singlet_cells]
```

```{r}
#HB53_background Doublet Process
...
# Run DoubletFinder with optimal parameters
HB53_background <- doubletFinder(HB53_background, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)

# Identify the column storing DoubletFinder classification
col_name <- colnames(HB53_background@meta.data)[8]

#output doublet plot/table
DimPlot(HB53_background, reduction = "umap.unintegrated", group.by = col_name)
table(HB53_background@meta.data$col_name)

# Create a logical vector for singlets
singlet_cells <- rownames(HB53_background@meta.data)[
  HB53_background@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB53_background <- HB53_background[, singlet_cells]
```

```{r}
#HB53_tumor Doublet Process
...
# Run DoubletFinder with optimal parameters
HB53_tumor <- doubletFinder(HB53_tumor, PCs = 1:10, pN = 0.25, pK = optimal_pK, nExp = nExp, sct = FALSE)



# Identify the column storing DoubletFinder classification
col_name <- colnames(HB53_tumor@meta.data)[8]

#output doublet plot/table
DimPlot(HB53_tumor, reduction = "umap.unintegrated", group.by = col_name)
table(HB53_tumor@meta.data$col_name)

# Create a logical vector for singlets
singlet_cells <- rownames(HB53_tumor@meta.data)[
  HB53_tumor@meta.data[[col_name]] == "Singlet"
]

# Subset manually
HB53_tumor <- HB53_tumor[, singlet_cells]
```
...
```{r}
#get cell and gene counts post filtering + doublet removal
HB17_background_cells_after <- ncol(HB17_background)
HB17_background_genes_after <- nrow(HB17_background)

HB17_PDX_cells_after <- ncol(HB17_PDX)
HB17_PDX_genes_after <- nrow(HB17_PDX)

HB17_tumor_cells_after <- ncol(HB17_tumor)
HB17_tumor_genes_after <- nrow(HB17_tumor)

HB30_PDX_cells_after <- ncol(HB30_PDX)
HB30_PDX_genes_after <- nrow(HB30_PDX)

HB30_tumor_cells_after <- ncol(HB30_tumor)
HB30_tumor_genes_after <- nrow(HB30_tumor)

HB53_background_cells_after <- ncol(HB53_background)
HB53_background_genes_after <- nrow(HB53_background)

HB53_tumor_cells_after <- ncol(HB53_tumor)
HB53_tumor_genes_after <- nrow(HB53_tumor)

```

```{r}
#Create table summarizing cell and gene counts before and after filtering 
qc_summary <- data.frame(
  Sample = c("HB17_background", "HB17_PDX", "HB17_tumor", 
             "HB30_PDX", "HB30_tumor", "HB53_background", "HB53_tumor"),
  ...
  Cells_After = c(HB17_background_cells_after, HB17_PDX_cells_after, HB17_tumor_cells_after,
                  HB30_PDX_cells_after, HB30_tumor_cells_after, HB53_background_cells_after, HB53_tumor_cells_after),
  
  Genes_After = c(HB17_background_genes_after, HB17_PDX_genes_after, HB17_tumor_genes_after,
                  HB30_PDX_genes_after, HB30_tumor_genes_after, HB53_background_genes_after, HB53_tumor_genes_after)
)
...
```
```{r}
save(qc_summary, file = "qc_summary.RData")
```
...
```{r}
#merge filtered data
merged_obj <- merge(HB17_background, y = c(HB17_PDX, HB17_tumor, HB30_PDX, HB30_tumor, HB53_background, HB53_tumor), add.cell.ids = c("HB17_background", "HB17_PDX", "HB17_tumor", "HB30_PDX", "HB30_tumor", "HB53_background", "HB53_tumor"), project = "all")
```
